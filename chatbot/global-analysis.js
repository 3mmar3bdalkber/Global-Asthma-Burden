/* ══════════════════════════════════════════════════════════════
   global-analysis.js — Global Asthma Intelligence Dashboard
   REBUILT to match the actual GBD_Asthma_Final.csv schema exactly:
   measure_name, location_name, sex_name, age_name, metric_name,
   year, val, upper, lower, val_display

   KEY CORRECTNESS RULES (previously violated — this is the fix):
   1. Always read `val_display`, not `val`. For Percent rows `val` is
      a raw fraction (e.g. 0.0131) while `val_display` is the actual
      human percentage (e.g. 1.31) — mixing them produced nonsense
      KPIs like "798% avg rate".
   2. Never sum across age_name groups that overlap. "All ages" is
      itself a total — summing it together with "0-14 years",
      "15-49 years" etc. double/triple counts the same deaths. Every
      aggregation below pins down an exact age_name.
   3. decade / risk_category are NOT used anywhere in this file, per
      request — everything is grouped by the real `year` column, and
      "risk" is shown as an actual prevalence ranking, not a derived
      bucket column.
   ══════════════════════════════════════════════════════════════ */
...
(function () {
  ...
  // Maps a logical "measure" the UI cares about onto the metric_name it needs,
  // plus how multiple rows should be combined. The measure_name itself is matched
  // fuzzily (see measureMatches) because real-world exports often use full
  // descriptive labels — "DALYs (Disability-Adjusted Life Years)" — or even typo
  // variants ("DAYLs...") instead of a short exact code. Exact-matching 'DALYs'
  // against 'DALYs (Disability-Adjusted Life Years)' silently returns zero rows,
  // which is exactly why the DALYs chart/KPI came back empty.
  const MEASURE_CFG = {
  ...
  function loadCSV(file) {
    ...
    Papa.parse(file, {
      ...
      dynamicTyping: true,
      skipEmptyLines: true,
      // worker:true intentionally not used — it silently never calls back under file://,
      // which caused "Processing dataset…" to hang forever.
      ...
      complete: function (results) {
        ...
        try {
          ...
          console.error('GlobalAnalysis render error:', err);
          statusEl.className = 'data-status err';
  ...
  function init() {
    ...
    statusEl.className = 'data-status ok';
    statusEl.style.display = 'block';
    statusEl.textContent = window.T('mock-loaded');
    renderStats();
    renderAll();
    wireControls();
  }
  ...
  window.GlobalAnalysis = {
    init, loadCSV, onThemeChange, onLangChange,
    getData: () => RAW,
    isMock: () => usingMock,
