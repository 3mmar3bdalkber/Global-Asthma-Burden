(function () {
  ...
  // patient.vw_patient_enriched returns snake_case columns (age, bmi, smoking,
  // lung_function_fev1, diagnosis...). The rest of this file was written against
  // the original CSV's PascalCase headers (Age, BMI, Smoking, LungFunctionFEV1,
  // Diagnosis, PatientID...) — this mapper bridges the two without touching any
  // other function below.
  function mapApiRowToPatient(r) {
    return {
      ...
      PollutionExposure: r.pollution_exposure,
      PollenExposure: r.pollen_exposure,
      DustExposure: r.dust_exposure,
      PetAllergy: r.pet_allergy,
      FamilyHistoryAsthma: r.family_history_asthma,
      HistoryOfAllergies: r.history_of_allergies,
      Eczema: r.eczema,
      HayFever: r.hay_fever,
      GastroesophagealReflux: r.gastroesophageal_reflux,
      LungFunctionFEV1: r.lung_function_fev1,
      LungFunctionFVC: r.lung_function_fvc,
      Wheezing: r.wheezing,
      ShortnessOfBreath: r.shortness_of_breath,
      ChestTightness: r.chest_tightness,
      Coughing: r.coughing,
      NighttimeSymptoms: r.nighttime_symptoms,
      ExerciseInduced: r.exercise_induced,
  ...
  async function loadFromAPI() {
    ...
    try {
      ...
      const res = await fetch(`${baseUrl}/api/patient/enriched`);
      if (!res.ok) throw new Error(`API ${res.status}: ${await res.text()}`);
      const rows = await res.json();

      const cleaned = rows
        .filter(r => r && r.patient_id !== null && r.patient_id !== undefined)
        .map(mapApiRowToPatient);

      if (!cleaned.length) throw new Error('0 usable rows returned from /api/patient/enriched');
      ...
      console.error('PatientML loadFromAPI error:', err);
      statusEl.className = 'data-status err';
      statusEl.textContent = `Could not reach backend — patient data not loaded. (${err.message})`;
    }
  }
  ...
  function renderTable() {
    ...
    const rows = PATIENTS.slice(0, 300); 
  ...
  window.PatientML = { init, loadFromAPI, onThemeChange, onLangChange, getData: () => PATIENTS, runPrediction };
})();
